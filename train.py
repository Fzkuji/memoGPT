...
#
# from models.memoryGPT.model import GPT
# from models.memoryGPT.config import MemoryConfig
#
# # -----------------------------------------------------------------------------
...
# model_args = dict(memory_dim=memory_dim, short_term_memory_size=short_term_memory_size,
...
#     gptconf = MemoryConfig(**model_args)
#     model = GPT(gptconf)
...
#             X, Y = get_batch(split, context_len=train_size if split == 'train' else val_size)
#             with ctx:
...
#
# # training loop
# X, Y = get_batch('train', train_size)  # fetch the very first batch
# t0 = time.time()
...
#         X, Y = get_batch('train', train_size)
...
master_process = local_rank == 0  # this process will do logging, checkpointing etc.
...
ds_config = {
    ...
    "pipeline": {
        ...
        "partition_count": world_size,  # Number of pipeline stages
...
t0 = time.time()
local_iter_num = 0  # number of iterations in the lifetime of this process
raw_model = model_engine.module if hasattr(model_engine, 'module') else model_engine  # unwrap DDP container if needed
running_mfu = -1.0
while True:
...
model_engine.save_checkpoint(out_dir)